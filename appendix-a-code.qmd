...
```bash
pixi install
pixi run kernel     # register Jupyter kernel (once)
pixi shell          # enter the env (3rd step for reliable render)
  ./render.sh
  exit
...
(See preface for why `pixi shell` before render is the 3rd bullet in the workflow.)
